...
class FundLoanModel {
  ...
  final String
  status; // 'pending' | 'approved' | 'rejected' | 'settled'  ('active' = legacy approved)
  ...
  bool get isSettled =>
      status == 'settled' || (_isApproved && outstanding <= 0.0001);
  ...
  factory FundLoanModel.fromMap(Map<String, dynamic> data, String id) {
    return FundLoanModel(
      ...
      repayments:
          ((data['repayments'] as List<dynamic>?) ?? [])
              .map(
                (e) => FundLoanRepayment.fromMap(
