...
class _PendingPaymentsReviewScreenState
  ...
  Widget build(BuildContext context) {
    final pendingAsync = ref.watch(
      pendingPaymentsStreamProvider(widget.groupId),
    );
    final expensesAsync = ref.watch(
      pendingExpensesStreamProvider(widget.groupId),
    );
      ...
      body: TabBarView(
        ...
        children: [
                ...
                child: pendingAsync.when(
                  data: (payments) {
                    _selectedIds.removeWhere(
                      (id) => !payments.any((p) => p.id == id),
                    );
                    if (payments.isEmpty) {
                      return _EmptyState(
                        label: 'No pending payments to review',
                      );
                    }
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed:
                                _isProcessing ? null : _showBatchRejectDialog,
          ...
          expensesAsync.when(
            data: (expenses) {
              if (expenses.isEmpty) {
                return _EmptyState(label: 'No pending expense requests');
              }
              return ListView.builder(
                padding: const EdgeInsets.all(DS.screenPad),
                      ...
                      child: StatefulBuilder(
                        builder: (context, setModalState) {
                          ...
                          return ElevatedButton.icon(
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor:
                                  isSelected ? DT.text : DT.surface,
                              foregroundColor:
                                  isSelected ? Colors.white : DT.text,
                              elevation: 0,
                              side:
                                  isSelected
                                      ? null
  ...
  Future<void> _quickApprove(PaymentModel payment) async {
    ...
    try {
      ...
      if (!mounted) return;
      showAppSnackBar('Payment approved');
    } catch (e) {
      if (!mounted) return;
      showAppSnackBar('Error: $e', isError: true);
    }
  ...
  Future<void> _quickReject(PaymentModel payment) async {
    ...
    try {
      ...
      if (!mounted) return;
      showAppSnackBar('Payment rejected', isError: true);
    } catch (e) {
      if (!mounted) return;
      showAppSnackBar('Error: $e', isError: true);
    }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 3),
                    Text(
                      '${expense.requestedByName}  ·  $dateStr${hasReceipt ? '  ·  Receipt' : ''}',
                      style: GoogleFonts.manrope(
                        fontSize: 12,
                        color: DT.textTertiary,
                      ),
