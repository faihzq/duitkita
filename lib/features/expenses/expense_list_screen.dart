...
class _ExpenseListScreenState extends ConsumerState<ExpenseListScreen> {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '"${e.title}" (RM${e.amount.toStringAsFixed(2)}) will be permanently removed.',
  ...
  Future<void> _batchOp(bool approve) async {
    ...
    try {
      ...
      if (approve) {
        await svc.batchApproveExpenses(
          expenseIds: _selected.toList(),
          approvedBy: uid,
          approvedByName: name,
        );
      } else {
        await svc.batchRejectExpenses(
          expenseIds: _selected.toList(),
          rejectedBy: uid,
          rejectedByName: name,
        );
                ...
                children: [
                  ...
                  if (!_selectionMode)
                    expensesAsync.maybeWhen(
                      data: (all) {
                        final years =
                            all.map((e) => e.createdAt.year).toSet().toList()
            ...
            SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _FilterChip(
                    label: 'Pending',
                    active: _filter == ExpenseStatus.pending,
                    activeColor: DT.warning,
                    onTap:
                        () => setState(() {
                          _filter = ExpenseStatus.pending;
                          _exitSelect();
                        }),
                  ),
                  const SizedBox(width: 8),
                  _FilterChip(
                    label: 'Approved',
                    active: _filter == ExpenseStatus.approved,
                    activeColor: DT.success,
                    onTap:
                        () => setState(() {
                          _filter = ExpenseStatus.approved;
                          _exitSelect();
                        }),
                  ),
                  const SizedBox(width: 8),
                  _FilterChip(
                    label: 'Rejected',
                    active: _filter == ExpenseStatus.rejected,
                    activeColor: DT.danger,
                    onTap:
                        () => setState(() {
                          _filter = ExpenseStatus.rejected;
                          _exitSelect();
                        }),
                  ),
                ],
                      ...
                      GestureDetector(
                        onTap: () {
                          if (expensesAsync.hasValue) {
                            _selectAll(expensesAsync.value!);
                          }
                        },
                        child: Text(
                          'Select all',
            ...
            Expanded(
              child: expensesAsync.when(
                ...
                error:
                    (e, _) => Center(
                      child: Padding(
                        ...
                        child: Text(
                          'Error: $e',
                          style: GoogleFonts.manrope(color: DT.danger),
                          textAlign: TextAlign.center,
                        ),
                      ),
                    ),
                data: (all) {
                  ...
                  if (_year != null) {
                    list =
                        list.where((e) => e.createdAt.year == _year).toList();
                  }
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(
                                color: DT.danger.withValues(alpha: 0.5),
                              ),
                              borderRadius: BorderRadius.circular(12),
                            ),
                            child: Center(
  ...
  void _openDetail(ExpenseModel e) {
    Navigator.of(context).push(
      AppTheme.slideRoute(
        ExpenseDetailScreen(expense: e, groupId: widget.groupId),
      ),
    );
  }
}
          ...
          IntrinsicHeight(
            child: Row(
              children: [
                _HeroStatCell(
                  label: 'Approved',
                  value: 'RM${approved.toStringAsFixed(0)}',
                  color: DT.accent,
                ),
                Container(
                  width: 1,
                ...
                _HeroStatCell(
                  label: 'Rejected',
                  value: 'RM${rejected.toStringAsFixed(0)}',
                  color: DT.danger,
                ),
              ],
            ),
...
class _ExpenseCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusLabel =
        expense.status.name[0].toUpperCase() + expense.status.name.substring(1);
    final dateStr =
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(DS.cardRadius),
        border: Border.all(
          color: selected ? DT.primary.withValues(alpha: 0.35) : DT.border,
                  ...
                  children: [
                    AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: selected ? DT.primary : DT.borderStrong,
                          width: 2,
                        ),
                      ),
                      child:
    ...
    children: [
      ...
      Text(
        isFiltered
            ...
            : 'Submit an expense request to get started',
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: active ? color.withValues(alpha: 0.4) : DT.border,
          ),
        ),
        child: Text(
          label,
          style: GoogleFonts.manrope(
            fontSize: 13,
            fontWeight: FontWeight.w700,
            color: active ? color : DT.textSecondary,
          ),
        ),
      ),
    );
