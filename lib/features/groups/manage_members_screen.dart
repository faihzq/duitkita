...
class _ManageMembersScreenState extends ConsumerState<ManageMembersScreen> {
  ...
  Future<void> _checkAdmin() async {
    ...
    final ok = await ref
        ...
        .isUserAdmin(widget.groupId, uid);
  ...
  Future<void> _sendInvite(String email) async {
    ...
    await SharePlus.instance.share(
      ShareParams(text: msg, subject: 'Join ${widget.groupName} on DuitKita'),
    );
  }

  void _shareInvite() {
    final msg =
        'Join "${widget.groupName}" on DuitKita! Track monthly payments with family.\n\n'
        'Download: https://appdistribution.firebase.dev/i/24015162eba1d1f9';
  ...
  Future<void> _removeMember(String userId, String userName) async {
    ...
    try {
      await ref
          .read(groupServiceProvider)
          .removeMemberFromGroup(groupId: widget.groupId, userId: userId);
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 2),
                                  Text(
                                    'RM${m.totalPaid.toStringAsFixed(2)} · ${m.paymentCount} payments',
                                    style: GoogleFonts.manrope(
                                      fontSize: 11,
                                      color: DT.textTertiary,
                            ...
                            if (_isAdmin && !isMe && !m.isAdmin)
                              GestureDetector(
                                onTap:
                                    () => _removeMember(m.userId, m.userName),
                                child: Container(
      ...
      child: Center(
        child: Text(
          ...
          style: GoogleFonts.manrope(
            fontSize: 15,
            fontWeight: FontWeight.w800,
            color: isAdmin ? DT.primary : DT.accentDeep,
          ),
        ),
      ),
    );
