...
class _BulkImportScreenState extends ConsumerState<BulkImportScreen> {
  ...
  List<({int year, int month})> _allMonths() {
    ...
    final groupStart = DateTime(
      widget.groupCreatedAt.year,
      widget.groupCreatedAt.month,
    );
    final start = janLastYear.isBefore(groupStart) ? janLastYear : groupStart;
    var cur = start;
    while (cur.year < now.year ||
        (cur.year == now.year && cur.month <= now.month)) {
  ...
  Future<void> _doImport() async {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'This will add $total payments totalling RM${(amount * total).toStringAsFixed(2)}.\nDuplicates are skipped automatically.',
                    style: GoogleFonts.manrope(
                      fontSize: 13,
    ...
    try {
      final entries =
          <({String userId, String userName, int month, int year})>[];
  ...
  void _snack(String msg, {bool isError = false}) {
    showSnackBar(context, msg, isError: isError);
  }
                      ...
                      children: [
                          ...
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                  color: done || active ? DT.accent : DT.border,
                                  width: 1.5,
                                ),
                                shape: BoxShape.circle,
                              ),
                        ...
                        if (i < 2)
                          Expanded(
                            child: Container(
                              height: 2,
                              color: i < _step ? DT.accent : DT.border,
                            ),
                          ),
                      ],
                    ),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  height: 22,
                  decoration: BoxDecoration(
                    color: allSelected ? DT.accent : DT.bg,
                    border: Border.all(
                      color: allSelected ? DT.accent : DT.border,
                      width: 1.5,
                    ),
        ...
        ...members.map((m) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: selected ? DT.accentSoft : DT.surface,
                border: Border.all(
                  color: selected ? DT.accent : DT.border,
    ...
    return Column(
      children: [
        ...
        _ReviewSection(
          ...
          children:
              (_selectedMonths.toList()..sort()).map((key) {
                final parts = key.split('-');
                return _ReviewChip(
                  label: '${_monthNames[int.parse(parts[1]) - 1]} ${parts[0]}',
                  color: DT.accent,
                );
              }).toList(),
        ),
    ...
    child: Center(
      child: Text(
        ...
        style: GoogleFonts.manrope(
          fontSize: 13,
          fontWeight: FontWeight.w800,
          color: selected ? Colors.white : DT.primary,
        ),
      ),
    ),
  );
