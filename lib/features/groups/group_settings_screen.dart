...
class _GroupSettingsScreenState extends ConsumerState<GroupSettingsScreen> {
  ...
  Future<void> _editGroup(GroupModel group) async {
    ...
    final amtCtrl = TextEditingController(
      text: group.monthlyAmount.toStringAsFixed(2),
    );
    final balCtrl = TextEditingController(
      text: group.initialBalance.toStringAsFixed(2),
    );
    final goalCtrl = TextEditingController(
      text: group.goalAmount > 0 ? group.goalAmount.toStringAsFixed(2) : '',
    );
    ...
    if (name.isEmpty) {
      _snack('Group name cannot be empty');
      return;
    }
    final amt = double.tryParse(amtCtrl.text.trim());
    final bal = double.tryParse(balCtrl.text.trim());
    final goal =
        goalCtrl.text.trim().isEmpty
            ? 0.0
  ...
  Future<void> _pickGroupIcon(GroupModel group) async {
    ...
    if (action.emoji != null) {
      ...
      try {
        ...
        await ref
            .read(groupServiceProvider)
            .setGroupIcon(groupId: widget.groupId, emoji: action.emoji);
    ...
    if (action.remove) {
      ...
      try {
        ...
        await ref
            .read(groupServiceProvider)
            .setGroupIcon(groupId: widget.groupId, emoji: '🏠');
        if (old != null) ref.read(storageServiceProvider).deleteGroupImage(old);
    ...
    if (action.pickPhoto) {
      try {
        ...
        final url = await ref
            ...
            .uploadGroupImage(groupId: widget.groupId, file: File(picked.path));
        await ref
            .read(groupServiceProvider)
            .setGroupIcon(groupId: widget.groupId, iconUrl: url);
        if (old != null) ref.read(storageServiceProvider).deleteGroupImage(old);
  ...
  Future<void> _setSettled(GroupModel group, bool value) async {
    ...
    try {
      await ref
          .read(groupServiceProvider)
          .updateGroup(groupId: widget.groupId, isSettled: value);
      if (mounted) {
        _snack(
          value
              ? 'Group settled — monthly collection stopped'
              : 'Group reopened',
        );
      }
    } catch (e) {
  ...
  Future<void> _setReminderDay(GroupModel group) async {
                ...
                (ctx, set) => Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Members will be reminded on this day each month.',
                        style: GoogleFonts.manrope(
                          fontSize: 13,
    ...
    try {
      await ref
          .read(groupServiceProvider)
          .updateGroup(groupId: widget.groupId, reminderDay: result);
  ...
  Future<void> _bankAccount(GroupModel group) async {
    ...
    final holderCtrl = TextEditingController(
      text: group.accountHolderName ?? '',
    );
    String? selectedBank =
        group.bankName != null && banks.contains(group.bankName)
            ? group.bankName
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    'Members will transfer payments to this account',
                                    style: GoogleFonts.manrope(
                                      fontSize: 12,
                                      color: DT.textSecondary,
                                    ),
    ...
    try {
      await ref
          ...
          .updateGroup(
            ...
            accountNumber:
                accCtrl.text.trim().isEmpty ? null : accCtrl.text.trim(),
            accountHolderName:
                holderCtrl.text.trim().isEmpty ? null : holderCtrl.text.trim(),
          );
  ...
  Future<void> _toggleAutoApprove(GroupModel group) async {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      'All existing pending payments will be confirmed automatically. You can also review them first.',
                      style: GoogleFonts.manrope(
                        fontSize: 13,
                        color: DT.textSecondary,
                        height: 1.4,
                    ...
                    Column(
                      children: [
                        ...
                        GestureDetector(
                          onTap: () => Navigator.pop(ctx, 'approve_all'),
                          child: Container(
                            width: double.infinity,
                            padding: const EdgeInsets.symmetric(vertical: 12),
                            decoration: BoxDecoration(
    ...
    try {
      await ref
          .read(groupServiceProvider)
          .updateGroup(groupId: widget.groupId, autoApprovePayments: val);
      if (val) {
      ...
      if (mounted) {
        _snack(val ? 'Auto-approve enabled' : 'Payments require review');
      }
    } catch (e) {
      if (mounted) _snack('Error: $e');
    } finally {
  ...
  Future<void> _toggleAutoApproveExpense(GroupModel group) async {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      'All pending expenses will be approved automatically. You can also review them first.',
                      style: GoogleFonts.manrope(
                        fontSize: 13,
                        color: DT.textSecondary,
                        height: 1.4,
    ...
    try {
      await ref
          .read(groupServiceProvider)
          .updateGroup(groupId: widget.groupId, autoApproveExpenses: val);
      if (val) {
        final uid = ref.read(authControllerProvider.notifier).currentUser?.uid;
        if (uid != null) {
          final profile = await ref
      ...
      if (mounted) {
        _snack(
          val ? 'Auto-approve expenses enabled' : 'Expenses require review',
        );
      }
    } catch (e) {
      if (mounted) _snack('Error: $e');
  ...
  Future<void> _deleteGroup(GroupModel group) async {
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'This is permanent. All members, payments, and data will be lost.',
                        style: GoogleFonts.manrope(
                          fontSize: 13,
                          color: DT.textSecondary,
                          height: 1.4,
                              ...
                              decoration: InputDecoration(
                                ...
                                border: OutlineInputBorder(
                                  ...
                                  borderSide: BorderSide(
                                    color: DT.danger.withValues(alpha: 0.4),
                                  ),
                                ),
                                enabledBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(12),
                                  borderSide: BorderSide(
                                    color: DT.danger.withValues(alpha: 0.3),
                                  ),
                                ),
                          ...
                          Expanded(
                            child: GestureDetector(
                              ...
                              child: AnimatedContainer(
                                ...
                                decoration: BoxDecoration(
                                  color: matches ? DT.danger : DT.dangerSoft,
                                  borderRadius: BorderRadius.circular(12),
                                ),
                                child: Center(
                                  child: Text(
    ...
    try {
      ...
      showAppSnackBar('Group deleted');
    } catch (e) {
      showAppSnackBar('Error: $e');
    }
  }

  void _snack(String msg) => showSnackBar(context, msg);
  ...
  Future<void> _exportCsv(GroupModel group) async {
    ...
    try {
      final payments = await ref.read(
        groupPaymentsStreamProvider(widget.groupId).future,
      );
      final expenses = await ref.read(
        groupExpensesStreamProvider(widget.groupId).future,
      );
      ...
      final buf =
          StringBuffer()..writeln(
            'Type,Date,Member,Description,Amount (RM),Status,Method,Reference',
          );
      ...
      final file = File(
        '${dir.path}/${safeName}_export_${DateTime.now().millisecondsSinceEpoch}.csv',
      );
      ...
      await SharePlus.instance.share(
        ShareParams(
          ...
          text:
              '${group.name}: ${payments.length} payment(s) and ${expenses.length} expense(s)',
        ),
      );
    } catch (e) {
  ...
  void _shareInvite(GroupModel group) {
    final msg =
        'Join "${group.name}" on DuitKita! Track monthly payments with family.\n\n'
        'Download: https://appdistribution.firebase.dev/i/24015162eba1d1f9';
    SharePlus.instance.share(
      ShareParams(text: msg, subject: 'Join ${group.name} on DuitKita'),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isAdmin =
        membersAsync.valueOrNull?.any((m) => m.userId == uid && m.isAdmin) ??
        false;
                    ...
                    children: [
                      ListView(
                        ...
                        children: [
                          ...
                          _Card(
                            children: [
                              _ValueRow(
                                ...
                                value:
                                    'RM ${group.monthlyAmount.toStringAsFixed(0)}',
                                onTap:
                                    (isAdmin && !_busy)
                                        ? () => _editGroup(group)
                                        : null,
                              ...
                              _ValueRow(
                                label: 'Reminder day',
                                value:
                                    '${_ordinal(group.reminderDay)} of month',
                                onTap:
                                    (isAdmin && !_busy)
                                        ? () => _setReminderDay(group)
                                        : null,
                              ),
                              _Divider(),
                              _ValueRow(
                                ...
                                value:
                                    group.goalAmount > 0
                                        ? 'RM ${group.goalAmount.toStringAsFixed(0)}'
                                        : 'None',
                                onTap:
                                    (isAdmin && !_busy)
                                        ? () => _editGroup(group)
                          ...
                          if (group.bankName != null ||
                              group.accountNumber != null ||
                              isAdmin) ...[
                            const SizedBox(height: DS.xl),
                            ...
                            _Card(
                              children: [
                                ...
                                _ToggleRow(
                                  ...
                                  subtitle:
                                      group.autoApproveExpenses
                                          ? 'Expenses approved instantly'
                                          : 'Requires admin review',
                                  ...
                                  onChanged:
                                      _busy
                                          ? null
                                          : (_) =>
                                              _toggleAutoApproveExpense(group),
                                ),
                                _Divider(),
                                _ToggleRow(
                                  icon: Icons.flag_circle_outlined,
                                  iconBg: DT.primarySoft,
                                  iconColor: DT.primary,
                                  label: 'Settled',
                                  subtitle:
                                      group.isSettled
                                          ? 'Goal reached — no monthly collection'
                                          : 'Still collecting monthly',
                                  value: group.isSettled,
                                  onChanged:
                                      _busy
                                          ? null
                                          : (v) => _setSettled(group, v),
                                ),
                              ],
                            ),
                            ...
                            _Card(
                              children: [
                                _SettingRow(
                                  ...
                                  label: 'Export to CSV',
                                  subtitle:
                                      'Payments + expenses for this group',
...
class _ManageAdminsSheetState extends ConsumerState<_ManageAdminsSheet> {
  ...
  Future<void> _promote(GroupMember m) async {
    final ok = await showDialog<bool>(
      ...
      builder:
          (ctx) => _ConfirmDialog(
            ...
            body:
                '${m.userName} will be able to manage settings, approve payments, and manage members.',
          ),
    );
    if (ok != true || !mounted) return;
    ...
    try {
      ...
      if (mounted) showSnackBar(context, '${m.userName} is now an admin');
    } catch (e) {
  ...
  Future<void> _demote(GroupMember m) async {
    ...
    try {
      ...
      if (mounted) showSnackBar(context, '${m.userName} is no longer an admin');
    } catch (e) {
      if (mounted) showSnackBar(context, 'Error: $e');
    } finally {
      if (mounted) setState(() => _processing = false);
                    ...
                    if (admins.isNotEmpty) ...[
                      ...
                      ...admins.map(
                        (m) => _MemberTile(
                          member: m,
                          isAdmin: true,
                          isCurrentUser: m.userId == widget.currentUserId,
                          adminCount: admins.length,
                          processing: _processing,
                          onPromote: () => _promote(m),
                          onDemote: () => _demote(m),
                        ),
                      ),
                    ],
...
class _MemberTile extends ConsumerWidget {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: GoogleFonts.manrope(
          fontSize: 14,
          fontWeight: FontWeight.w800,
          color: isAdmin ? DT.primary : DT.textSecondary,
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final photoUrl =
        ref
            .watch(userProfileStreamProvider(member.userId))
            .valueOrNull
            ?.profileImageUrl;
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isAdmin ? DT.primary.withValues(alpha: 0.2) : DT.border,
        ),
      ),
      child: Row(
        children: [
                ...
                Row(
                  children: [
                    ...
                    if (isAdmin) ...[
                      ...
                      _Pill('Admin', DT.primarySoft, DT.primary),
                    ],
                    if (isCurrentUser) ...[
                      const SizedBox(width: 6),
                      _Pill('You', DT.accentSoft, DT.accentDeep),
                    ],
                  ],
                ),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Text(
      ...
      style: GoogleFonts.manrope(
        fontSize: 11,
        fontWeight: FontWeight.w700,
        color: danger ? DT.danger : DT.textTertiary,
        letterSpacing: 0.5,
      ),
    ),
  );
...
class _Divider extends StatelessWidget {
  @override
  Widget build(BuildContext context) =>
      const Divider(height: 1, indent: 16, endIndent: 16, color: DT.border);
}
      ...
      child: Row(
        children: [
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.manrope(
                    fontSize: 14,
                    fontWeight: FontWeight.w700,
                    color: dangerous ? DT.danger : DT.text,
                  ),
                ),
          ...
          Icon(
            ...
            size: 18,
            color:
                dangerous ? DT.danger.withValues(alpha: 0.5) : DT.textTertiary,
          ),
        ],
      ),
    ),
      ...
      children: [
        ...
        Transform.scale(
          ...
          child: Switch(
            value: value,
            onChanged: onChanged,
            activeThumbColor: DT.success,
            activeTrackColor: DT.successSoft,
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: DT.text,
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(color: DT.surface, width: 3),
                    ),
                    child: Icon(
                      iconUrl != null
          ...
          children: [
                  ...
                  children: [
                    for (int i = 0; i < preview.length; i++)
                      Positioned(
                        left: i * 24.0,
                        child: _StackAvatar(
                          userId: preview[i].userId,
                          name: preview[i].userName,
                        ),
                      ),
                  ],
                ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    'Manage ${members.length} member${members.length == 1 ? '' : 's'}',
                    style: GoogleFonts.manrope(
                      fontSize: 14,
                      fontWeight: FontWeight.w700,
                      color: DT.text,
                  ...
                  const SizedBox(height: 2),
                  Text(
                    '$adminCount admin${adminCount == 1 ? '' : 's'} · ${members.length - adminCount} member${members.length - adminCount == 1 ? '' : 's'}',
                    style: GoogleFonts.manrope(
                      fontSize: 11,
                      color: DT.textSecondary,
                    ),
        ...
        children: [
          Text(
            ...
            style: GoogleFonts.manrope(
              fontSize: 17,
              fontWeight: FontWeight.w800,
              color: destructive ? DT.danger : DT.text,
            ),
          ),
          const SizedBox(height: 10),
          Text(
