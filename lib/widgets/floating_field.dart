...
class _FloatingFieldState extends State<FloatingField> {
  ...
  Widget build(BuildContext context) {
    ...
    final iconColor = hasError ? DT.danger : (active ? DT.text : DT.textTertiary);

    return Padding(padding: EdgeInsets.only(bottom: widget.gap), child: Column(
      ...
      children: [
        AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: DT.surface, borderRadius: BorderRadius.circular(14),
            border: Border.all(color: borderColor, width: 1.5),
            boxShadow: (focused && !hasError)
                ? [BoxShadow(color: DT.primary.withValues(alpha: 0.08), blurRadius: 8, spreadRadius: 4)]
                : null),
          child: Row(crossAxisAlignment: multiline ? CrossAxisAlignment.start : CrossAxisAlignment.center, children: [
            Padding(padding: EdgeInsets.only(top: multiline ? 12 : 0),
              child: widget.leadingOverride ?? Icon(widget.icon, size: 18, color: iconColor)),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: AnimatedDefaultTextStyle(
                  ...
                  style: GoogleFonts.manrope(
                    fontSize: active ? 10 : 14, fontWeight: active ? FontWeight.w700 : FontWeight.w500,
                    color: active ? (focused ? DT.text : DT.textSecondary) : DT.textTertiary,
                    letterSpacing: active ? 0.4 : 0),
                  child: Text(active ? '${widget.label.toUpperCase()}${widget.optional ? ' · OPTIONAL' : ''}'
                                      : '${widget.label}${widget.optional ? ' · optional' : ''}'),
                )),
                if (widget.showCounter && widget.maxLength != null && active)
              ...
              TextField(
                ...
                keyboardType: widget.keyboardType, textCapitalization: widget.capitalization,
                inputFormatters: widget.inputFormatters, maxLines: widget.maxLines,
                ...
                decoration: InputDecoration(
                  ...
                  prefixStyle: GoogleFonts.manrope(fontSize: 15, fontWeight: FontWeight.w800, color: DT.textTertiary),
        ...
        if (hasError || widget.helper != null) ...[
          ...
          Padding(padding: const EdgeInsets.only(left: 6), child: Row(children: [
            ...
            Text(hasError ? widget.error! : widget.helper!,
              style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w600, color: hasError ? DT.danger : DT.textTertiary)),
          ])),
        ],
      ],
    ));
