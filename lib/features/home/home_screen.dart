...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final groups = ref.watch(userGroupsStreamProvider(user.uid)).valueOrNull ?? [];
    final debts = ref.watch(userDebtsStreamProvider(user.uid)).valueOrNull ?? [];

    final fullName = profile?.name ?? user.displayName ?? user.email?.split('@').first ?? 'there';
    ...
    for (final g in groups) {
      final status = ref.watch(groupMonthPaymentStatusProvider((groupId: g.id, userId: user.uid))).valueOrNull;
    ...
    final progressPct = totalMonthly > 0 ? (paidThisMonth / totalMonthly).clamp(0.0, 1.0) : 0.0;
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: Row(
                      children: [
                        GestureDetector(
                          ...
                          child: _Avatar(name: fullName, imageUrl: profileImageUrl, size: 42),
                        ),
                        ...
                        const SizedBox(width: 12),
                        _HelpButton(onTap: () => _showHelp(context, ref, user.uid)),
                        const SizedBox(width: 12),
                  ...
                  Padding(
                    ...
                    child: _QuickActions(context: context, userId: user.uid, onTabChange: onTabChange),
                  ),
...
class _NotificationBell extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final memberNotifs = ref.watch(userPaymentNotificationsProvider(userId)).valueOrNull?.length ?? 0;
    final adminGroupIds = (ref.watch(adminGroupIdsStreamProvider(userId)).valueOrNull ?? {}).toList();
    final paymentService = ref.watch(paymentServiceProvider);

    return StreamBuilder<List<dynamic>>(
      stream: adminGroupIds.isNotEmpty
      ...
      builder: (context, snapshot) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: Stack(
              children: [
                const Center(child: Icon(Icons.notifications_outlined, size: 20, color: DT.text)),
                if (total > 0)
                  Positioned(
                    top: 8,
                    right: 9,
  ...
  void _showNotifications(BuildContext context, WidgetRef ref, String userId) {
    ...
    final adminGroupIds = (ref.read(adminGroupIdsStreamProvider(userId)).valueOrNull ?? {}).toList();
    final memberNotifs = ref.read(userPaymentNotificationsProvider(userId)).valueOrNull ?? [];
            ...
            children: [
              ...
              if (adminGroupIds.isNotEmpty) ...[
                StreamBuilder<List<dynamic>>(
                  stream: paymentService.getPendingPaymentsForGroupsStream(adminGroupIds),
                  builder: (context, snapshot) {
                    ...
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _notifHeader('Pending Payments', pending.length, DT.warning),
                        ...pending.map((p) => _notifTile(
                          context: context,
                          icon: Icons.hourglass_top_rounded,
                          color: DT.warning,
                          title: '${p.userName} — RM${p.amount.toStringAsFixed(2)}',
                          subtitle: groupNameMap[p.groupId] ?? 'Unknown group',
                          time: p.createdAt,
                          onTap: () {
                            Navigator.pop(context);
                            Navigator.of(context).push(AppTheme.slideRoute(
                              PendingPaymentsReviewScreen(groupId: p.groupId, groupName: groupNameMap[p.groupId] ?? ''),
                            ));
                          },
                        )),
                      ],
                ...
                StreamBuilder<List<dynamic>>(
                  stream: expenseService.getPendingExpensesForGroupsStream(adminGroupIds),
                  builder: (context, snapshot) {
                    final pending = snapshot.data ?? [];
                    if (pending.isEmpty) return const SizedBox.shrink();
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _notifHeader('Pending Expenses', pending.length, DT.danger),
                        ...pending.map((e) => _notifTile(
                          context: context,
                          icon: Icons.receipt_long_outlined,
                          color: DT.danger,
                          title: '${e.title} — RM${e.amount.toStringAsFixed(2)}',
                          subtitle: '${e.requestedByName} · ${groupNameMap[e.groupId] ?? ''}',
                          time: e.createdAt,
                          onTap: () {
                            Navigator.pop(context);
                            Navigator.of(context).push(AppTheme.slideRoute(
                              ExpenseListScreen(groupId: e.groupId, groupName: groupNameMap[e.groupId] ?? ''),
                            ));
                          },
                        )),
                      ],
              ...
              if (memberNotifs.isNotEmpty) ...[
                _notifHeader('Payment Updates', memberNotifs.length, DT.catGroups),
                ...memberNotifs.map((p) {
                  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                  final isConfirmed = p.paymentStatus == 'confirmed';
                  return _notifTile(
                    context: context,
                    icon: isConfirmed ? Icons.check_circle : Icons.cancel,
                    color: isConfirmed ? DT.success : DT.danger,
                    title: '${months[p.month - 1]} ${p.year} — ${isConfirmed ? 'Confirmed' : 'Rejected'}',
                    subtitle: groupNameMap[p.groupId] ?? 'Unknown group',
                    time: p.verifiedAt ?? p.createdAt,
                    onTap: () {
                      Navigator.pop(context);
                      Navigator.of(context).push(AppTheme.slideRoute(GroupDetailScreen(groupId: p.groupId)));
                    },
                  );
                }),
              ],
              if (memberNotifs.isEmpty && adminGroupIds.isEmpty)
                Padding(
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        const Icon(Icons.notifications_none, size: 48, color: DT.textTertiary),
                        const SizedBox(height: 12),
                        Text('No notifications', style: GoogleFonts.manrope(fontSize: 14, color: DT.textTertiary)),
                      ],
                    ),
                  ),
                ),
  ...
  static Widget _notifTile({
    ...
    final label = diff.inMinutes < 60 ? '${diff.inMinutes}m ago' : diff.inHours < 24 ? '${diff.inHours}h ago' : '${diff.inDays}d ago';
    return InkWell(
      onTap: onTap,
      child: Padding(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(title, style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w600, color: DT.text)),
                  const SizedBox(height: 2),
                  Text(subtitle, style: GoogleFonts.manrope(fontSize: 11, color: DT.textSecondary)),
                ],
              ),
            ),
            Text(label, style: GoogleFonts.manrope(fontSize: 11, color: DT.textTertiary)),
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: RadialGradient(
                  colors: [DT.accent.withValues(alpha: 0.19), Colors.transparent],
                  stops: const [0.0, 0.7],
                ),
              ),
            ),
            ...
            children: [
                  ...
                  if (totalMonthly > 0)
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          Icon(
                            onTrack ? Icons.trending_up_rounded : Icons.trending_down_rounded,
                            size: 12,
              ...
              if (totalMonthly > 0) ...[
                ...
                Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: GoogleFonts.manrope(fontSize: 12, color: Colors.white.withValues(alpha: 0.85)),
                    ),
                    Text(
                ...
                Text(
                  ...
                  style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.white.withValues(alpha: 0.7)),
                ),
              ],
            ],
          ),
...
class _QuickActions extends StatelessWidget {
  ...
  final String userId;
  final void Function(int)? onTabChange;
  const _QuickActions({required this.context, required this.userId, this.onTabChange});
  ...
  Widget build(BuildContext _) {
    final actions = [
      ...
      _Action(Icons.pie_chart_outline_rounded, 'Insights', DT.catBillsSoft, DT.catBills, () {
        showModalBottomSheet(
          context: context,
          isScrollControlled: true,
          backgroundColor: Colors.transparent,
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w600, color: DT.text, letterSpacing: -0.1),
            ),
          ],
        ),
      ),
...
class _UpNextSection extends ConsumerWidget {
  ...
  Widget build(BuildContext buildContext, WidgetRef ref) {
    ...
    for (final g in groups.take(3)) {
      final status = ref.watch(groupMonthPaymentStatusProvider((groupId: g.id, userId: userId))).valueOrNull;
      if (status != 'confirmed') {
        items.add(_UpNextItemData(
          ...
          ctaLabel: 'Pay',
          muted: false,
          onTap: () => Navigator.of(context).push(AppTheme.slideRoute(GroupDetailScreen(groupId: g.id))),
        ));
      }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(item.title, style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w700, color: DT.text, letterSpacing: -0.2)),
                const SizedBox(height: 1),
                Text(item.subtitle, style: GoogleFonts.manrope(fontSize: 12, color: DT.textSecondary)),
              ],
            ),
          ),
          Column(
            ...
            children: [
              Text(
                ...
                style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w800, color: DT.text, letterSpacing: -0.3),
              ),
          ...
          child: Column(
            children: [
              if (activeGroups > 0) ...[
                ...
                if (activeDebts > 0 || activeBills > 0) const _BreakdownDivider(),
              ],
              if (activeDebts > 0) ...[
                _BreakdownRow(
                  icon: Icons.account_balance_outlined,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(sub, style: GoogleFonts.manrope(fontSize: 11, color: DT.textTertiary)),
                ],
              ),
            ),
            Text(
              'RM ${_fmtComma(amount)}',
              style: GoogleFonts.manrope(fontSize: 15, fontWeight: FontWeight.w800, color: DT.text, letterSpacing: -0.3),
            ),
                ...
                children: [
                  _SectionHeader(
                    ...
                    action: Container(
                      ...
                      decoration: BoxDecoration(color: DT.warningSoft, borderRadius: BorderRadius.circular(999)),
                      child: Text('${payments.length}', style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w700, color: DT.warning)),
                    ),
                  ),
                  ...payments.take(5).map((p) => Padding(
                    padding: const EdgeInsets.only(bottom: 8),
                    child: _AdminTile(
                      icon: Icons.hourglass_top_rounded,
                      ...
                      title: p.userName,
                      subtitle: '${groupNameMap[p.groupId] ?? 'Group'} · ${p.paymentDate.day}/${p.paymentDate.month}/${p.paymentDate.year}',
                      amount: p.amount,
                      onTap: () => Navigator.of(context).push(AppTheme.slideRoute(
                        PendingPaymentsReviewScreen(groupId: p.groupId, groupName: groupNameMap[p.groupId] ?? ''),
                      )),
                    ),
                  )),
              ...
              child: Column(
                ...
                children: [
                  _SectionHeader(
                    ...
                    action: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                      decoration: BoxDecoration(color: DT.dangerSoft, borderRadius: BorderRadius.circular(999)),
                      child: Text('${expenses.length}', style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w700, color: DT.danger)),
                    ),
                  ),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(title, style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w700, color: DT.text), overflow: TextOverflow.ellipsis),
                  const SizedBox(height: 2),
                  Text(subtitle, style: GoogleFonts.manrope(fontSize: 11, color: DT.textSecondary)),
                ],
              ),
            ...
            Container(
              ...
              child: Text('RM${amount.toStringAsFixed(2)}', style: GoogleFonts.manrope(fontSize: 12, fontWeight: FontWeight.w800, color: iconColor)),
            ),
            const SizedBox(width: 4),
            const Icon(Icons.chevron_right, size: 16, color: DT.textTertiary),
          ],
...
class _InsightsSheet extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final groups = ref.watch(userGroupsStreamProvider(userId)).valueOrNull ?? [];
    ...
    for (final g in groups) {
      final status = ref.watch(groupMonthPaymentStatusProvider((groupId: g.id, userId: userId))).valueOrNull ?? 'unpaid';
    ...
    final progress = totalMonthly > 0 ? (totalPaid / totalMonthly).clamp(0.0, 1.0) : 0.0;
          ...
          children: [
            ...
            Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Insights', style: GoogleFonts.manrope(fontSize: 22, fontWeight: FontWeight.w800, color: DT.text, letterSpacing: -0.4)),
                Text(monthLabel, style: GoogleFonts.manrope(fontSize: 13, color: DT.textSecondary)),
              ])),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                decoration: BoxDecoration(color: DT.primarySoft, borderRadius: BorderRadius.circular(DS.chipRadius)),
                child: Text(monthLabel, style: GoogleFonts.manrope(fontSize: 12, fontWeight: FontWeight.w700, color: DT.primary)),
              ),
            ]),
            const SizedBox(height: 20),
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('Total paid', style: GoogleFonts.manrope(fontSize: 12, color: Colors.white.withValues(alpha: 0.7))),
                  Text('of RM${_fmt(totalMonthly)}', style: GoogleFonts.manrope(fontSize: 12, color: Colors.white.withValues(alpha: 0.7))),
                ]),
                const SizedBox(height: 4),
                Text('RM${_fmt(totalPaid)}', style: GoogleFonts.manrope(fontSize: 28, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: -0.5)),
                const SizedBox(height: 12),
                ClipRRect(
                ...
                const SizedBox(height: 8),
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('${(progress * 100).toStringAsFixed(0)}% cleared', style: GoogleFonts.manrope(fontSize: 11, color: Colors.white.withValues(alpha: 0.7))),
                  if (remaining > 0)
                    Text('RM${_fmt(remaining)} remaining', style: GoogleFonts.manrope(fontSize: 11, color: DT.accent)),
                ]),
              ]),
            ),
            const SizedBox(height: 20),
            ...
            // Category breakdown
            if (totalMonthly > 0) ...[
              Text('Category breakdown', style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w700, color: DT.text)),
              const SizedBox(height: 10),
              Container(
                padding: const EdgeInsets.all(DS.lg),
                decoration: BoxDecoration(color: DT.surfaceAlt, borderRadius: BorderRadius.circular(DS.cardRadius), border: Border.all(color: DT.border)),
                child: Column(children: [
                  if (groupMonthly > 0) _CategoryBar(icon: Icons.group_rounded, label: 'Groups', paid: paidGroups, total: groupMonthly, color: DT.catGroups, softColor: DT.catGroupsSoft),
                  if (groupMonthly > 0 && debtMonthly > 0) const SizedBox(height: 12),
                  if (debtMonthly > 0) _CategoryBar(icon: Icons.account_balance_rounded, label: 'Loans', paid: paidDebts, total: debtMonthly, color: DT.catDebts, softColor: DT.catDebtsSoft),
                  if (debtMonthly > 0 && billMonthly > 0) const SizedBox(height: 12),
                  if (billMonthly > 0) _CategoryBar(icon: Icons.receipt_rounded, label: 'Bills', paid: paidBills, total: billMonthly, color: DT.catBills, softColor: DT.catBillsSoft),
                ]),
              ),
              const SizedBox(height: 20),
            ],
            ...
            if (groups.isNotEmpty) ...[
              Text('Groups', style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w700, color: DT.text)),
              const SizedBox(height: 10),
              ...groups.map((g) {
                final status = groupStatuses[g.id] ?? 'unpaid';
                final (Color c, Color s, String lbl, IconData ic) = switch (status) {
                  'confirmed' => (DT.success,      DT.successSoft, 'Paid',    Icons.check_circle_rounded),
                  'pending'   => (DT.warning,      DT.warningSoft, 'Pending', Icons.hourglass_top_rounded),
                  _           => (DT.textTertiary, DT.surfaceAlt,  'Unpaid',  Icons.radio_button_unchecked_rounded),
                };
                return Container(
                  ...
                  child: Row(children: [
                    Container(width: 36, height: 36, decoration: BoxDecoration(color: DT.catGroupsSoft, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.group_rounded, size: 18, color: DT.catGroups)),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(g.name, style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w700, color: DT.text), overflow: TextOverflow.ellipsis),
                      Text('RM${_fmt(g.monthlyAmount)}/mo', style: GoogleFonts.manrope(fontSize: 11, color: DT.textSecondary)),
                    ])),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(color: s, borderRadius: BorderRadius.circular(DS.chipRadius)),
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        Icon(ic, size: 12, color: c),
                        const SizedBox(width: 4),
                        Text(lbl, style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w700, color: c)),
                      ]),
                    ),
                  ]),
                );
            ...
            // Debts & bills
            if (debts.isNotEmpty) ...[
              Text('Loans & Bills', style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w700, color: DT.text)),
              const SizedBox(height: 10),
              ...debts.map((d) {
                final paid = debtPaid[d.id] ?? false;
                final (Color c, Color s, String lbl) = paid
                ...
                return Container(
                  ...
                  child: Row(children: [
                    Container(
                      width: 36, height: 36,
                      decoration: BoxDecoration(color: d.isDebt ? DT.catDebtsSoft : DT.catBillsSoft, borderRadius: BorderRadius.circular(10)),
                      child: Icon(d.isDebt ? Icons.account_balance_rounded : Icons.receipt_rounded, size: 18, color: d.isDebt ? DT.catDebts : DT.catBills),
                    ),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(d.title, style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w700, color: DT.text), overflow: TextOverflow.ellipsis),
                      Text('RM${_fmt(d.monthlyPayment)}/mo', style: GoogleFonts.manrope(fontSize: 11, color: DT.textSecondary)),
                    ])),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(color: s, borderRadius: BorderRadius.circular(DS.chipRadius)),
                      child: Text(lbl, style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w700, color: c)),
                    ),
                  ]),
                );
              }),
            ...
            if (totalMonthly == 0)
              Padding(
                ...
                child: Column(children: [
                  Container(padding: const EdgeInsets.all(20), decoration: const BoxDecoration(color: DT.primarySoft, shape: BoxShape.circle), child: const Icon(Icons.pie_chart_outline_rounded, size: 40, color: DT.primary)),
                  const SizedBox(height: 16),
                  Text('No commitments yet', style: GoogleFonts.manrope(fontSize: 15, fontWeight: FontWeight.w600, color: DT.textSecondary)),
                  const SizedBox(height: 4),
                  Text('Add a group or debt to see insights', style: GoogleFonts.manrope(fontSize: 13, color: DT.textTertiary)),
                ]),
              ),
          ],
        ),
...
class _CategoryBar extends StatelessWidget {
  ...
  const _CategoryBar({required this.icon, required this.label, required this.paid, required this.total, required this.color, required this.softColor});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      Row(children: [
        Container(width: 28, height: 28, decoration: BoxDecoration(color: softColor, borderRadius: BorderRadius.circular(8)), child: Icon(icon, size: 14, color: color)),
        const SizedBox(width: 10),
        Expanded(child: Text(label, style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w600, color: DT.text))),
        Text('RM${paid.toStringAsFixed(0)} / RM${total.toStringAsFixed(0)}', style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w600, color: DT.textSecondary)),
      ]),
      const SizedBox(height: 6),
      ClipRRect(
        borderRadius: BorderRadius.circular(4),
        child: LinearProgressIndicator(value: pct, minHeight: 6, backgroundColor: softColor, valueColor: AlwaysStoppedAnimation<Color>(color)),
      ),
    ]);
  }
}
