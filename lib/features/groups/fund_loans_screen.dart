...
class FundLoansScreen extends ConsumerWidget {
            ...
            Expanded(
              child: loansAsync.when(
                ...
                data: (loans) {
                  ...
                  final loaned = approved.fold<double>(
                    0,
                    (s, l) => s + l.principal,
                  );
                  final repaid = approved.fold<double>(
                    0,
                    (s, l) => s + l.amountRepaid,
                  );
                  final outstanding = approved.fold<double>(
                    0,
                    (s, l) => s + l.outstanding,
                  );
                  ...
                  return ListView(
                    ...
                    children: [
                      ...
                      if (loans.isEmpty)
                        _EmptyState(
                          isAdmin: isAdmin,
                          onAdd: () => _openAddLoan(context, ref),
                        )
                      else ...[
                        if (pending.isNotEmpty) ...[
                          _sectionLabel(
                            isAdmin
                                ? 'Requests · needs approval'
                                : 'Pending approval',
                          ),
                          ...pending.map(card),
  ...
  void _openAddLoan(BuildContext context, WidgetRef ref) {
    final members =
        ref.read(groupMembersStreamProvider(groupId)).valueOrNull ?? [];
    final myId =
  ...
  Future<void> _approve(
    ...
    try {
      await ref
          .read(fundLoanServiceProvider)
          .approveFundLoan(loanId: loan.id, approvedByName: myName);
      showAppSnackBar(
        'Approved — RM${loan.principal.toStringAsFixed(2)} drawn from the fund',
      );
    } catch (e) {
  ...
  Future<void> _reject(
    ...
    final ok = await _confirm(
      context,
      'Reject request?',
      'Reject "${loan.title}" from ${loan.borrowerName}? It won\'t affect the fund balance.',
      'Reject',
    );
    if (ok != true) return;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            '$count active loan${count != 1 ? 's' : ''} drawn from the fund',
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${loan.borrowerName} · borrowed RM${loan.principal.toStringAsFixed(2)}',
                        style: GoogleFonts.manrope(
                          fontSize: 11,
                          color: DT.textSecondary,
                        ),
                ...
                if (!loan.isPending && !loan.isRejected)
                  Column(
                    ...
                    children: [
                      Text(
                        'RM${loan.outstanding.toStringAsFixed(2)}',
                        style: GoogleFonts.manrope(
                          fontSize: 15,
                          fontWeight: FontWeight.w800,
                          color: loan.isSettled ? DT.textTertiary : DT.text,
                        ),
                      ),
                      Text(
                        loan.isSettled ? 'settled' : 'left',
                        style: GoogleFonts.manrope(
                          fontSize: 10,
                          color: DT.textTertiary,
                        ),
            ...
            if (loan.affectsBalance) ...[
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  ...
                  valueColor: AlwaysStoppedAnimation(
                    loan.isSettled ? DT.success : DT.catDebts,
                  ),
                ),
              ),
              const SizedBox(height: 8),
              Row(
                children: [
                  Text(
                    'RM${loan.amountRepaid.toStringAsFixed(2)} repaid',
                    style: GoogleFonts.manrope(
                      fontSize: 11,
                      color: DT.textSecondary,
                  ...
                  if (loan.monthlyRepayment > 0 && !loan.isSettled)
                    Text(
                      'RM${loan.monthlyRepayment.toStringAsFixed(0)}/mo · ${loan.monthsRemaining} mo left',
                      style: GoogleFonts.manrope(
                        fontSize: 11,
                        color: DT.textTertiary,
                      ),
        ...
        return [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _btn('Approve', DT.success, Colors.white, onApprove),
              ),
            ],
          ),
        ];
    ...
    if (loan.isActive && isAdmin) {
      return [
        ...
        Row(
          children: [
            Expanded(
              child: _btn('Record repayment', DT.text, Colors.white, onRepay),
            ),
            const SizedBox(width: 8),
...
class _AddLoanSheetState extends ConsumerState<_AddLoanSheet> {
  ...
  Future<void> _save() async {
    ...
    if (_titleCtrl.text.trim().isEmpty || amount <= 0 || _borrowerId == null) {
      showSnackBar(context, 'Enter a purpose, amount, and borrower');
      return;
    }
    setState(() => _busy = true);
    ...
    try {
      ...
      if (mounted) {
        ...
        showSnackBar(context, '$e', isError: true);
      }
    }
  }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              widget.isAdmin ? 'New Fund Loan' : 'Request Fund Loan',
              style: GoogleFonts.manrope(
                fontSize: 18,
                fontWeight: FontWeight.w800,
                color: DT.text,
            ...
            Text(
              widget.isAdmin
                  ? 'Money drawn from the fund, repaid over time'
                  : 'Sent to an admin for approval before the money is drawn',
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                alignment: Alignment.center,
                decoration: BoxDecoration(
                  color: _busy ? DT.textTertiary : DT.text,
                ...
                child:
                    _busy
                        ...
                        : Text(
                          widget.isAdmin ? 'Create loan' : 'Submit request',
                          style: GoogleFonts.manrope(
                            fontSize: 15,
                            fontWeight: FontWeight.w800,
                            color: Colors.white,
...
class _RepaySheetState extends ConsumerState<_RepaySheet> {
  ...
  void initState() {
    super.initState();
    final suggested =
        widget.loan.monthlyRepayment > 0
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '${widget.loan.title} · RM${widget.loan.outstanding.toStringAsFixed(2)} outstanding',
            style: GoogleFonts.manrope(fontSize: 12, color: DT.textSecondary),
          ),
          const SizedBox(height: 18),
          Text(
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              alignment: Alignment.center,
              decoration: BoxDecoration(
                color: _busy ? DT.textTertiary : DT.success,
                borderRadius: BorderRadius.circular(12),
              ),
              child:
                  _busy
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Text(
                isAdmin ? 'Add fund loan' : 'Request fund loan',
                style: GoogleFonts.manrope(
                  fontSize: 14,
                  fontWeight: FontWeight.w800,
                  color: Colors.white,
