...
class _ExpenseDetailScreenState extends ConsumerState<ExpenseDetailScreen> {
  ...
  /// Lets the requester withdraw their own pending request, and with it the
  /// receipt they uploaded. Without this the only way to remove an expense you
  /// submitted was to delete your whole account.
  Future<void> _confirmDelete() async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => _DeleteRequestDialog(expense: widget.expense),
    );
    if (confirmed != true || !mounted) return;

    setState(() => _isLoading = true);
    try {
      // Receipt first: deleteExpense only removes the Firestore document, so
      // dropping that first would orphan the upload in Storage with nothing
      // left pointing at it. Best-effort — a missing or already-deleted file
      // must not block removing the request itself.
      final receiptUrl = widget.expense.receiptUrl;
      if (receiptUrl != null && receiptUrl.isNotEmpty) {
        try {
          await ref.read(storageServiceProvider).deleteReceipt(receiptUrl);
        } catch (e) {
          debugPrint('Could not delete receipt for ${widget.expense.id}: $e');
        }
      }
      await ref.read(expenseServiceProvider).deleteExpense(widget.expense.id);
      if (mounted) {
        Navigator.of(context).pop();
        showSnackBar(context, 'Request deleted');
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
        showSnackBar(context, 'Could not delete: $e', isError: true);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isAdmin =
        membersAsync.valueOrNull?.any((m) => m.userId == userId && m.isAdmin) ??
        false;
    final isPending = expense.status == ExpenseStatus.pending;
    final isRequester = userId != null && userId == expense.requestedBy;
    final requesterIsAdmin =
        membersAsync.valueOrNull?.any(
          (m) => m.userId == expense.requestedBy && m.isAdmin,
        ) ??
        false;
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Withdrawing your own request. Deliberately limited to
                      // pending: once an admin has approved it the expense is
                      // part of the group's accounting, and letting one member
                      // erase that would leave the balance wrong for everyone
                      // else. Rejected requests stay as a record of the
                      // decision.
                            ...
                            child: Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  const SizedBox(width: 6),
                                  // The amount makes this label variable-width,
                                  // and a four-figure expense overflows the
                                  // button on a narrow phone. Scale down rather
                                  // than ellipsise — truncating the sum on the
                                  // approve button is the one thing that must
                                  // stay legible.
                                  Flexible(
                                    child: FittedBox(
...
class _ConfirmScreenState extends State<_ConfirmScreen> {
  ...
  void _appendQuickNote(String text) {
    ...
    widget.rejectNoteCtrl.selection = TextSelection.collapsed(
      offset: widget.rejectNoteCtrl.text.length,
    );
  }
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText:
                              'e.g. Receipt unclear, please resubmit with itemized photo',
...
/// Confirmation for deleting your own pending request.
///
/// Names the receipt explicitly when there is one — it is the part people do
/// not expect to lose, and it cannot be recovered afterwards.
class _DeleteRequestDialog extends StatelessWidget {
  final ExpenseModel expense;
  const _DeleteRequestDialog({required this.expense});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Text(
              'Your request for RM${expense.amount.toStringAsFixed(2)} will be '
              'removed and the admin will no longer see it'
              '${hasReceipt ? ', along with the receipt you uploaded' : ''}. '
...
class _RequesterCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final photoUrl =
        ref
            .watch(userProfileStreamProvider(expense.requestedBy))
            .valueOrNull
            ?.profileImageUrl;
    ...
    final allExpenses =
        ref.watch(groupExpensesStreamProvider(expense.groupId)).valueOrNull ??
        [];
    final theirs = allExpenses.where(
      (e) => e.requestedBy == expense.requestedBy && e.id != expense.id,
    );
    final approvedCount =
        theirs.where((e) => e.status == ExpenseStatus.approved).length;
    final rejectedCount =
        theirs.where((e) => e.status == ExpenseStatus.rejected).length;
                  ...
                  child:
                      photoUrl != null && photoUrl.isNotEmpty
                          ? Image.network(
                            ...
                            fit: BoxFit.cover,
                            errorBuilder:
                                (_, __, ___) => _initialsTile(initials),
                          )
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      requesterIsAdmin ? 'Admin' : 'Group Member',
                      style: GoogleFonts.manrope(
                        fontSize: 11,
                        color: DT.textSecondary,
                        fontWeight: FontWeight.w500,
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(
      children: [
        ...
        const SizedBox(width: 8),
        // Expanded, or the column takes its intrinsic width and overflows the
        // half-width box these sit in — "Past rejections" does not fit beside
        // the icon on a 390pt-wide phone.
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
...
class _HistoryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = <
      ({IconData icon, Color bg, Color color, String title, String subtitle})
    >[
      (
...
class _ReviewRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      border: Border(
        bottom: last ? BorderSide.none : const BorderSide(color: DT.border),
      ),
    ),
    child: Row(
